...
export const ScrollManager = (props: ScrollManagerProps) => {
    ...
    useFrame(() => {
        ...
        // 这行代码计算当前的 section 索引。data.scroll.current 是一个介于 0 到 1 之间的值，表示用户滚动的相对位置。data.pages 是总的页面数量或滚动区段数。将这两个值相乘后取整，可以得到当前用户所在的 section。
        ...
        if (data.scroll.current > lastScroll.current) {
            // 检测向下滚动
            if (curSection === 0 && data.scroll.current >= nextSectionStart) {
                // 如果当前区段是0，并且滚动位置超过了第二个区段的起始点
                onSectionChange(2);
            }
        } else if (data.scroll.current < lastScroll.current) {
            // 检测向上滚动
            if (curSection === 1 && data.scroll.current < nextSectionStart) {
